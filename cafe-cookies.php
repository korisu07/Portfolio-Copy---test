<!DOCTYPE html>
<html lang="ja">
<head>

<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
<meta name="description" content="ポートフォリオ「Cafe*Cookies ページ」の紹介ページです。">
<meta name="keywords" content="Portfolio,ポートフォリオ,Web制作,自主制作,個人制作">

<title>Works Cafe*Cookies - Y.K.Portfolio</title>

<?php include './inc/head.php'; ?>

</head>

<body id="works-page">
  <div class="container main-bg-img">
    <div class="content">

      <?php include './inc/header.php'; ?>

      <section>
        <h2>Works</h2>
        <h3 id="web">《Cafe*Cookies》</h3>
        <div class="section-box">

        <div class="mb20">
          <span class="mr20"><a href="/web/cafe/index.html" target="_blank">実際のページはこちら</a></span>
          <span>※別タブで開きます。</span>
        </div>

          <div class="preview-box">
            <div class="preview-img">
              <img src="img/works/cafeCookies_preview.jpg" alt="cafe site">
            </div>
            <div class="preview-text">
              <p>制作時間：4時間</p>
              <p class="mb20">ターゲット層：20代</p>
              
                <p>シェアボタンのテスト用に作りました。架空のカフェサイトを想定しています。</p>
                <p class="mb20">ファーストビューのみ。</p>
                
                <p>明るいカラーリングを意識して制作しました。</p>
            </div>
          </div> <!-- /.preview-box -->
        </div> <!-- section-box -->
          
      </section>
    </div> <!-- /.content -->


    <?php include './inc/footer.php'; ?>

  </div> <!-- /.container -->


</body>
</html>